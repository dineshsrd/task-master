...
import com.taskmaster.user.constant.UserConstants;
...
public class UserService {
    ...
    public ResponseModel register(UserModel user) {
        ...
        try {
            if (_userRepository.findByEmail(user.getEmail()).isPresent()) {
                throw new UserLevelException(UserConstants.USER_EXISTS);
            }
            ...
            message = UserConstants.USER_REGISTERED;
            status = HttpStatus.CREATED;
        } catch (Exception e) {
            taskMasterUser = null;
            message = e.getMessage();
            status = HttpStatus.BAD_REQUEST;
            LOGGER.severe(UserConstants.EXCEPTION_CREATE_USER);
        }
    ...
    public ResponseEntity<ResponseModel> fetchUser(Long userId, HttpServletRequest request){
        ...
        if (fetchedUser == null) {
            ...
            response.setMessage(UserConstants.USER_FETCHED);
    ...
    public ResponseEntity<ResponseModel> updateUser(Long userId, UserModel user) {
        ...
        try {
            ...
            if (fetchedUser == null) {
                response.setMessage(UserConstants.USER_NOT_FOUND);
                response.setStatus(HttpStatus.NOT_FOUND);
                return new ResponseEntity<>(response, HttpStatus.NOT_FOUND);
                ...
                response.setMessage(GeneralConstants.UNAUTHORIZED_ACCESS + " " + UserConstants.USER_UPDATE_ERROR);
    ...
    public ResponseEntity<ResponseModel> deleteUser(Long userId) {
        ...
        try {
            ...
            response.setMessage(UserConstants.USER_DELETED);
            response.setStatus(HttpStatus.OK);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception ex) {
            response.setMessage(ex.getMessage() + " " + UserConstants.ADMIN_ONLY_ACTION);
            response.setStatus(HttpStatus.BAD_REQUEST);
            return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
        }
    }
